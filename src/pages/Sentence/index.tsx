...
function Sentence() {
	...
	const words = currentSentence ? currentSentence.translation.trim().split(/\s+/) : [];

	// Update inputValue when sentence changes or completes
	useEffect(() => {
		if (currentSentence) {
			if (currentSentence.rate === parseInt(inputValue)) {
				setInputValue(currentSentence.rate === 1 ? "2" : "1");
			}
			if (currentSentence.rate === 0 && inputValue === "2") {
				setInputValue("1");
			}
		}
	}, [currentSentence, inputValue]); // Added inputValue to dependencies though it might cause loops? We keep it similar to ReviewCard
	
	// Better yet, just use a separate effect for resetting on new sentence
	useEffect(() => {
			...
			<div className="content">
				...
				<div className="flashcard">
					<div className="language-indicator">
						<span className="category-label">Sentence Practice</span>
					</div>

					<div className="card-content">
						<div className="side">
							<h2>{currentSentence.original}</h2>
						</div>
